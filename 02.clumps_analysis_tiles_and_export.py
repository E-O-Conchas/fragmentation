...
This script processes fragmentation maps for various habitats, generating clump maps, reports, 

and exporting them to GeoTiff format. It runs in a GRASS GIS environment, assuming the fragmentation 

maps and region settings are correctly set. The script performs the following tasks for each habitat:
...
for habitat in habitats:
    ...
    base_output_path = rf'S:\Emmanuel_OcegueraConchas\fragmentation_maps_tiles_and_input\EUNIS\{habitat}\{year}'
    ...
    for map_type, subfolder in frag_map_types:
        ...
        map_name = f"{habitat}_{year}_{map_type}_Map"
    ...
    try:
        ...
        for map_name, output_path, clump_prefix in frag_maps_list:
            ...
            clum_map_name = map_name.replace('Fragmentation', 'bfragmap').replace('_Map', '_clump')
